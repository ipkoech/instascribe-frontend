...
export class WebsocketsService implements OnDestroy {
  ...
  connect() {
    try {
      if (!this.cable) {
        this.cable = ActionCable.createConsumer(`${this.api.base_uri}cable?token=${this.getCookie('access_token')}`);
  ...
  subscribeAndListenToChannel(channelName: string, params: {}, callback: (data: any) => void) {
  ...
  private installEventListeners() {
    // Window state events
    window.addEventListener('focus', this.updateBound, { passive: true });
    window.addEventListener('blur', this.updateBound, { passive: true });
    ...
    // Optional: Add online/offline detection
    window.addEventListener('online', () => this.connect(), { passive: true });
    window.addEventListener('offline', () => this.disconnected(), { passive: true });
  }
  ...
  private uninstallEventListeners() {
    ...
    // Document visibility cleanup
    document.removeEventListener('visibilitychange', this.handleVisibilityChange.bind(this));

    // Optional: Clean up online/offline listeners
  ...
  private appear() {
    for (let channelName in this.channels) {
      this.channels[channelName].perform('appear', { appearing_on: this.appearingOn() });
    }
  }
